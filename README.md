...
每个`.png`图像文件都有一个对应的同名`.xml`标注文件。例如:
- `0027.png`
- `0027.xml`
...
- 其他文件：
  ...
  - `fileState.txt`：列出了每张图像的文件路径及其处理状态。1 表示该图片已被正确处理或标注完成。
  - `Label.txt`：包含了每张图片的标注信息，包括识别的车厢号字符、位置坐标。

### 数据目的

该数据集主要用于：
1. 训练车厢号识别模型，识别车厢号和车厢型号的具体字符信息。
2. 提供图像中车厢号字符的位置信息和识别难度标注，帮助优化模型的检测和识别能力。
